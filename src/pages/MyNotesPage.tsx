...
export default function MyNotesPage() {
  ...
  // 検索文字列(初期 '')。並べ替えキー(初期 'word-asc'=英語 A→Z)。
  const [query, setQuery] = useState('');
  const [sortKey, setSortKey] = useState<NoteSortKey>('word-asc');
  ...
  // notes を NoteRow[] に写像する。空文字/空白のみのメモは「メモが付いている」とは
  // 見なさないので除外する。entry が無い(孤児)場合は english/japanese は null になる。
  // 並べ替えは filterAndSortNoteRows が担うので、ここでは wordId 昇順で安定させる。
  const noteRows = useMemo<NoteRow[]>(() => {
    return Object.entries(notes)
      ...
      .map(([wordId, note]) => ({
        wordId,
        note,
        english: dictById.get(wordId)?.english ?? null,
        japanese: dictById.get(wordId)?.japanese ?? null,
      }))
      .sort((a, b) => a.wordId.localeCompare(b.wordId));
  }, [notes, dictById]);

  // 全体の件数(検索で絞り込まれないベースライン)。
  const count = noteRows.length;

  // 検索 + 並べ替えを通した表示用行。
  const visibleRows = useMemo(
    () => filterAndSortNoteRows(noteRows, query, sortKey),
    [noteRows, query, sortKey],
  );
  const visibleCount = visibleRows.length;

  // 検索で絞り込まれているか(クエリ非空 かつ 表示件数が全体より少ない)。
  const isFiltered = query.trim() !== '' && visibleCount < count;
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {/* 検索で絞り込まれているときは『〇件中 △件を表示』を添える */}
      {isFiltered && (
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
          {count} 件中 {visibleCount} 件を表示
        </p>
      )}

      {/* メモ一覧: 検索結果0件のときは専用メッセージ(空状態の辞書導線とは別物) */}
      {visibleCount === 0 ? (
        ...
        <div className="space-y-3 mb-8">
          {visibleRows.map(({ wordId, note, english, japanese }) => {
            // 辞書に一致する entry が無い wordId(孤児)でも本文は表示する。
            // NoteRow の english/japanese は entry が無いとき null になる。
            const isOrphan = english === null;
            const displayEnglish = english ?? '(辞書に存在しない単語)';
            const displayJapanese = japanese ?? '';
            // aria-label 接頭辞: 辞書一致なら english、孤児なら wordId で一意に。
            const ariaPrefix = english ?? wordId;
