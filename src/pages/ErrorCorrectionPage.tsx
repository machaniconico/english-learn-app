...
import { errorCorrectionSets } from '../data/toeic/error-correction';
...
export default function ErrorCorrectionPage() {
  ...
  if (setId) {
    const set = errorCorrectionSets.find((s) => s.id === setId);
    ...
    return (
      <div>
        ...
        {/* Quiz */}
        <ErrorCorrection questions={set.questions} />
  ...
  return (
    <div>
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">誤り訂正 - 文法エラーを見つけよう</p>
        <p className="mt-3 text-gray-500 dark:text-gray-400 max-w-md mx-auto text-sm leading-relaxed">
          文中の文法的な誤りを含む箇所を4つの選択肢から見つけ出しましょう。TOEIC対策に最適です。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 pb-10">
        {errorCorrectionSets.map((set) => {
          const config = levelConfig[set.level];

          return (
            <Link
              key={set.id}
              to={`/error-correction/${set.id}`}
