    ...
    return (
      <div className="w-full max-w-lg mx-auto text-center">
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-lg p-8 sm:p-10">
          ...
          <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-2">Results</h2>
          <p className="text-gray-500 dark:text-gray-400 mb-6">Part 2 {'\u7D50\u679C'}</p>
            ...
            {results.map((correct, i) => (
              <div
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
                  correct ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300'
                }`}
              >
                {correct ? '\u25CB' : '\u2715'}
              </div>
          ...
          <div className="text-left bg-gray-50 dark:bg-gray-900 rounded-xl p-4 mb-8">
              ...
              {Object.entries(typeStats).map(([type, stat]) => (
                <div key={type} className="flex items-center justify-between text-sm">
                  <span className="inline-flex items-center gap-2">
                    <span className="inline-block px-2 py-0.5 text-xs font-semibold rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300">
                      {type}
                    </span>
                  </span>
                  <span
          ...
          <button
            ...
            className="px-6 py-3 rounded-xl bg-indigo-600 dark:bg-indigo-500 text-white font-semibold hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-colors cursor-pointer shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 dark:focus:ring-offset-gray-900"
          >
            {'\u3082\u3046\u4E00\u5EA6\u6311\u6226'}
          </button>
        </div>
  ...
  return (
    <div className="w-full max-w-lg mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <span className="inline-block px-3 py-1 text-xs font-bold rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
        ...
        <div className="flex items-center justify-center gap-3 mb-4">
          <button
            ...
            className={`w-14 h-14 rounded-full inline-flex items-center justify-center text-2xl transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 ${
              speaking && !playingAll
                ? 'bg-indigo-200 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 animate-pulse ring-2 ring-indigo-400'
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 ${
              playingAll
                ? 'bg-purple-200 dark:bg-purple-900/60 text-purple-700 dark:text-purple-300 animate-pulse'
                : 'bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 hover:bg-purple-200 dark:hover:bg-purple-900/60 active:scale-95'
            }`}
          >
            {'\u25B6'} Play All
          </button>
        ...
        {current.responses.map((response, index) => {
          ...
          if (answered) {
            if (index === current.correctIndex) {
              ...
              optionStyle = 'bg-red-50 dark:bg-red-900/40 border-red-400 dark:border-red-700 text-red-800 dark:text-red-300 ring-2 ring-red-300 dark:ring-red-700';
            } else {
          ...
          return (
            <div key={index} className="flex items-stretch gap-2">
              ...
              <button
                ...
                disabled={speaking}
                className="w-10 shrink-0 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 flex items-center justify-center text-sm hover:bg-indigo-50 dark:hover:bg-indigo-900/40 hover:border-indigo-200 dark:hover:border-indigo-800 transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2"
                ...
                <span className="inline-flex items-center gap-3">
                  ...
                  <span>
                    ...
                    {answered || showText ? (
                      ...
                      <span className="text-gray-400 dark:text-gray-500">({LABELS[index]})</span>
                    )}
                  </span>
                </span>
              </button>
