...
interface ConversationListeningProps {
  conversations: Part3Question[];
}

const OPTION_LABELS = ['A', 'B', 'C', 'D'] as const;

export default function ConversationListening({ conversations }: ConversationListeningProps) {
  ...
  const totalQuestions = conversations.reduce((sum, c) => sum + c.questions.length, 0);
  ...
  const speakText = useCallback(
    (text: string, opts?: { pitch?: number; voiceName?: string }, onEnd?: () => void) => {
  ...
  const playConversation = useCallback(() => {
    ...
    const lines = current.conversation;
    let idx = 0;

    const speakerVoice = (speaker: string): { pitch: number; voiceName?: string } => {
      const lower = speaker.toLowerCase();
      if (lower.includes('woman') || lower.includes('female') || lower === 'speaker 2') {
        return { pitch: 1.2, voiceName: 'Samantha' };
      }
      return { pitch: 0.85, voiceName: 'Daniel' };
    };

    const playNext = () => {
      if (cancelRef.current || idx >= lines.length) {
        setPlaying(false);
        setPlayingLine(null);
        return;
      }
    ...
    playNext();
  }, [current.conversation, speakText, stop]);
          ...
          <div className="mb-8">
            ...
            <div className="space-y-2">
              {conversations.map((conv, ci) => {
                ...
                const convResults = results.slice(startIdx, startIdx + conv.questions.length);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
          ...
          >
            {playing ? (
              ...
              <>
                ...
                {hasPlayed ? 'もう一度聞く' : 'Play Conversation'}
              </>
            )}
          </button>
        </div>
      ...
      {/* After all questions answered: show next conversation button */}
      {allQuestionsAnswered && (
