  ...
  const renderSentence = () => {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (['A', 'B', 'C', 'D'].includes(part)) {
        ...
        let segmentStyle =
          'underline decoration-2 decoration-indigo-400 underline-offset-4 cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900/40 px-1 py-0.5 rounded transition-colors';

        if (state.answered) {
          if (segIdx === currentQuestion.errorIndex) {
            segmentStyle =
              'underline decoration-2 decoration-green-500 underline-offset-4 bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-300 px-1 py-0.5 rounded font-semibold';
          } else if (segIdx === state.selectedIndex && segIdx !== currentQuestion.errorIndex) {
            segmentStyle =
              'underline decoration-2 decoration-red-500 underline-offset-4 bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-300 px-1 py-0.5 rounded line-through';
          } else {
            segmentStyle =
              'underline decoration-2 decoration-gray-300 dark:decoration-gray-600 underline-offset-4 text-gray-400 dark:text-gray-500 px-1 py-0.5 rounded';
          }
        }

        elements.push(
          <span key={`label-${segIdx}`} className="text-xs font-bold text-indigo-500 dark:text-indigo-400 align-super mx-0.5">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-lg p-8 sm:p-10">
                ...
                {wrongAnswers.map((result) => (
                  ...
                  >
                    ...
                    <div className="text-sm mb-2">
                      <span className="font-medium text-green-700 dark:text-green-300">Correction:</span>{' '}
                      <span className="text-green-800 dark:text-green-300">{result.question.correction}</span>
                    </div>
          ...
          <div className="text-center">
            <button
              ...
              className="px-6 py-3 rounded-xl bg-indigo-600 dark:bg-indigo-500 text-white font-semibold hover:bg-indigo-700 dark:hover:bg-indigo-400 transition-colors cursor-pointer shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 dark:focus:ring-offset-gray-800"
            >
              もう一度
            </button>
          </div>
        ...
        {currentQuestion.segments.map((segment, index) => {
          ...
          if (state.answered) {
            if (index === currentQuestion.errorIndex) {
              ...
              btnStyle = 'bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500';
            }
          }
          ...
          return (
            <button
              ...
              `}
            >
              <span className="block text-xs font-bold text-indigo-500 dark:text-indigo-400 mb-1">
                {state.answered
                  ? index === currentQuestion.errorIndex
                    ? '\u2713'
                    : index === state.selectedIndex
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <div className="text-sm text-gray-700 dark:text-gray-300 mb-2">
              ...
              {' \u2192 '}
              <span className="text-green-700 dark:text-green-300 font-semibold">{currentQuestion.correction}</span>
            </div>
