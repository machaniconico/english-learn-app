    ...
    return (
      <div className="w-full max-w-lg mx-auto text-center">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-8 sm:p-10">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">発音練習 完了</h2>
          <div
            role="status"
          ...
          <p className="text-sm text-gray-500 mb-8">{scores.length}フレーズの平均一致率</p>
          <button
            type="button"
            onClick={restart}
  ...
  return (
    <div className="w-full max-w-lg mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6 sm:p-8 mb-6 text-center">
        ...
        <p className="text-2xl sm:text-3xl font-bold text-gray-800 leading-relaxed">
          ...
          {result
            ? result.words.map((w, i) => (
                <span
                  key={i}
                  className={w.matched ? 'text-green-600' : 'text-red-500'}
                >
                  {w.word}
        ...
        <p className="text-base text-gray-600 mt-2">{current.japanese}</p>
        <p className="text-sm text-gray-400 mt-1">{current.pronunciation}</p>
      ...
      {!supported ? (
        ...
        <div className="text-center mb-6">
          <button
            ...
            className={`w-20 h-20 rounded-full inline-flex items-center justify-center text-4xl transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 ${
              listening
      ...
      {result && (
        <div className="text-center" role="status" aria-live="polite">
          <p className="text-sm text-gray-500 mb-1">あなたの発話</p>
          <p className="text-base font-medium text-gray-700 mb-3">「{transcript}」</p>
          <p className="mb-4">
          ...
          <div className="flex justify-center gap-3">
            <button
              type="button"
              onClick={record}
              className="px-5 py-2.5 rounded-lg bg-white border border-indigo-200 text-indigo-600 font-medium hover:bg-indigo-50 transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-300"
            >
              もう一度話す
            </button>
            <button
              type="button"
              onClick={next}
              className="px-5 py-2.5 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition-colors cursor-pointer shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2"
            >
      ...
      {!result && (
        <div className="text-center">
          <button
            type="button"
            onClick={next}
            className="text-sm text-gray-400 hover:text-gray-600 underline transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300 rounded"
          >
            スキップ
          </button>
        </div>
